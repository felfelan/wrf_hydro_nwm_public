      ...
      if (.not.((tokenCount.eq.5).or.(tokenCount.eq.6))) then
        call ESMF_LogSetError(ESMF_FAILURE, &
          msg="Malformed ocn_export_fields item FORMAT="// &
            "'STATE_NAME' 'STANDARD_NAME' 'UNITS' 'IMPORT' 'EXPORT' "// & !            "['FILLVAL'] "//
            "in file: "//trim(fname), &
            rcToReturn=rc)
        return ! bail out
      endif
...
!   cleanup
 ...
 !-----------------------------------------------------------------------------
  ...
  subroutine state_copy_tohyd(state, did, rc)
    ...
    integer                                :: stat
    character(len=16), parameter :: method = "state_copy_tohyd"
  ...
  subroutine state_copy_frhyd(state, did, rc)
    ...
    character(len=16), parameter :: method = "state_copy_frhyd"
  ...
  subroutine state_check_missing(state, did, rc)
    ...
    logical                                :: missng
    integer                                :: stat
    character(len=19), parameter :: method = "state_check_missing"
  ...
  subroutine state_prescribe_missing(state, did, fieldList, rc)
    ...
    real(ESMF_KIND_R8)                     :: filVal
    integer                                :: stat
    character(len=23), parameter :: method = "state_prescribe_missing"
  ...
  subroutine model_debug(state, did, memflg, filePrefix, rc)
    ...
    integer                                :: stat
    character(len=16)                      :: cmemflg
    character(len=11), parameter :: method = "model_debug"
